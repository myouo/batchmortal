...
- 天凤牌谱 ID 自带日本时区的小时和规则位，因此文件模式会将其转换为 UTC，恢复小时精度的
  近似开始时间以及四/三麻、东/南场模式，并按实际模式保存到 `mode_<mode>` 目录。
- 雀魂共享链接本身没有精确开始/结束时间或房间模式；UUID 的日期前缀也不足以无歧义地换算
  为 UTC。因此文件模式将 `startTime`/`endTime` 留空并保存到 `mode_file`，图表只用 UTC
  执行时间维持稳定顺序，不会把执行时间显示成对局时间。精确元信息需要读取官方
  `fetchGameRecord` 响应中的 `head`。
...
所有新写入的非空 `startTime`、`endTime` 和执行 `timestamp` 都使用 RFC 3339 UTC
（例如 `2026-08-29T11:47:23Z`），不会随运行机器的本地时区变化。
